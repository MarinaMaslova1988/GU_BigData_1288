...
-- Создание таблиц, внешних ключей. Внесение данных в таблицы.
...
/* Задание 1. Проанализировать запросы, которые выполнялись на занятии, определить возможные корректировки и/или улучшения (С ПРИМЕНЕНИЕМ JOIN).*/

-- 1.1 На уроке рассматривался следующий запрос:
SELECT user_id, friend_id FROM friendship
WHERE user_id=5 AND friend_id in
	(SELECT user_id FROM friendship
    WHERE friend_id=5);
    
-- С помощью JOIN этот запрос можно перезаписать следующим образом:
SELECT fr.user_id, fr.friend_id FROM friendship fr
INNER JOIN friendship fr2 ON (fr.friend_id = fr2.user_id)
WHERE fr.user_id=5 
AND fr2.friend_id=5;



-- 1.2 На уроке рассматривался следующий запрос:
...
-- С помощью JOIN этот запрос можно перезаписать следующим образом:
-- (использую в данном запросе DISTINCT, поскольку у пользователей (id = 5 и 88) есть общие друзья (иными словами: чтобы убрать дубли).
	
SELECT DISTINCT p.* FROM friendship fr
INNER JOIN profiles p on fr.friend_id = p.user_id
WHERE fr.user_id in (5,88)
ORDER BY p.user_id 

/*ЗАДАНИЕ 2. Пусть задан некоторый пользователь. 
Из всех друзей этого пользователя найдите человека, который больше всех общался с нашим пользователем.*/

SELECT a.friend_id, count(*) FROM friendship a
INNER join messages mes on a.friend_id = mes.from_user_id
WHERE a.user_id =5 and a.friendship_status_id=1 -- для примера взят пользователь с user_id=5
AND mes.to_user_id = 5
GROUP BY a.friend_id-- выводим друзей пользователя и количество сообщений с сортировкой по убыванию. Самая первая строка показывает нам друга, который больше всего общался с пользователем 
ORDER BY count(*) DESC;
...
-- Добавляем данные в таблицы posts_likes и media_likes (для выполнения задания)
...
INSERT INTO `media_likes` (`media_id`, `user_id`, `created_at`) VALUES (26, 91, '1993-06-22 18:43:41');
INSERT INTO `media_likes` (`media_id`, `user_id`, `created_at`) VALUES (26, 92, '2018-01-07 14:21:31');
INSERT INTO `media_likes` (`media_id`, `user_id`, `created_at`) VALUES (44, 91, '1993-06-22 18:43:41');
INSERT INTO `media_likes` (`media_id`, `user_id`, `created_at`) VALUES (44, 82, '2018-01-07 14:21:31');
INSERT INTO `media_likes` (`media_id`, `user_id`, `created_at`) VALUES (93, 91, '1993-06-22 18:43:41');
INSERT INTO `media_likes` (`media_id`, `user_id`, `created_at`) VALUES (93, 92, '2018-01-07 14:21:31');
INSERT INTO `media_likes` (`media_id`, `user_id`, `created_at`) VALUES (93, 97, '1993-06-22 18:43:41');
INSERT INTO `media_likes` (`media_id`, `user_id`, `created_at`) VALUES (93, 82, '2018-01-07 14:21:31');

/*Для решения данной задачи создала 2 запроса (один - для подсчета лайков постов, второй - для подсчета лайков медиафайлов). 
После сделала JOIN обоих запросов. 
*/

SELECT sum(AA.post_like_count), sum(BB.media_like_count) FROM
	(SELECT a.user_id, count(c.post_id) AS post_like_count FROM
		(SELECT *  
		FROM profiles
		ORDER BY birthday desc
		limit 10) AS a -- мне нужен ПОЛНЫЙ список самых молодых пользователей (поэтому используется LEFT JOIN - не все пользователи делали посты)
		LEFT JOIN user_posts b on a.user_id=b.user_id
		LEFT JOIN posts_likes c on b.id = c.post_id
		group by a.user_id
	 ) AS AA	 
  INNER JOIN  
	(SELECT a.user_id, count(c.media_id) as media_like_count FROM
		(SELECT *  
		FROM profiles
		ORDER BY birthday desc
		limit 10) AS a -- мне нужен ПОЛНЫЙ список самых молодых пользователей (поэтому используется LEFT JOIN - не все пользователи загружали медиафайлы)
		LEFT JOIN media b on a.user_id=b.user_id
		LEFT JOIN media_likes c on b.id = c.media_id
		group by a.user_id
	  ) AS BB	  
  ON AA.user_id = BB.user_id; -- сделан JOIN таблиц по списку самых молодых пользователей (в обоих запросах список идентичен)
...
-- Решение задачи: 
  ...
  SELECT a.gender, (a.posts_likes_count+b.media_likes_count) as LIKES_COUNT FROM 
		(SELECT pp.gender, count(*) as posts_likes_count FROM posts_likes l -- подсчет количества лайков постов с группировкой по полу
		INNER JOIN profiles pp on l.user_id = pp.user_id
		GROUP BY pp.gender
        ) as a
  INNER JOIN
		(SELECT pp.gender, count(*) as media_likes_count FROM media_likes l -- подсчет количества лайков на медиафайлы с группировкой по полу
		INNER JOIN profiles pp on l.user_id = pp.user_id
		GROUP BY pp.gender
        ) as b
   on a.gender = b.gender -- связываю оба запроса по полу
   GROUP BY a.gender;
...
Исходя из этого нужно искать пользователей, которые меньше всего выполняет данные операции */
    ...
    SELECT u.user_id, (count(mes.id)+count(p.post_id)+count(m.media_id)+count(pos.id)+count(b.id)) as a
    FROM profiles u
	LEFT JOIN messages mes on u.user_id = mes.from_user_id -- сообщения
	LEFT JOIN posts_likes p on u.user_id = p.user_id -- лайки на посты
	LEFT JOIN media_likes m on u.user_id = m.user_id -- лайки на медиафайлы
	LEFT JOIN user_posts pos on u.user_id = pos.user_id -- выкладывание постов
