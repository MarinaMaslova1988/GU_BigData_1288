...
/*РЕШЕНИЕ  */
/***************************************** Создание БД для выполнения ДЗ. Создание таблиц и связей между таблицами. *******************************************************/

create database vk_lesson_10;
use vk_lesson_10;
...
-- Универсальная таблица лайков
CREATE TABLE likes (
  id int unsigned NOT NULL AUTO_INCREMENT COMMENT 'Идентификатор лайка',
  target_id int unsigned NOT NULL COMMENT 'Ссылка на контент',
  target_type_id int unsigned NOT NULL COMMENT 'Ссылка на контент',
  from_user_id int unsigned NOT NULL COMMENT 'Ссылка на пользователя кто поставил лайк',
  like_status tinyint DEFAULT NULL COMMENT 'Лайк или дизлайк',
  created_at datetime DEFAULT current_timestamp() COMMENT 'Время создания строки',
  updated_at datetime DEFAULT current_timestamp() ON UPDATE current_timestamp() COMMENT 'Время обновления строки',
  PRIMARY KEY (id)
);

-- Словарь "лайкабельных" сущностей
CREATE TABLE target_types (
	`id` int unsigned NOT NULL AUTO_INCREMENT COMMENT 'Идентификатор лайкабельного объекта',
    `table_name` varchar(100) NOT NULL COMMENT 'Наименование сущности лайкабельного объекта',
    PRIMARY KEY (id)
);
...
ALTER TABLE likes ADD CONSTRAINT fk_likes_target_types FOREIGN KEY (target_type_id) REFERENCES target_types (id);
ALTER TABLE likes ADD CONSTRAINT fk_likes_user FOREIGN KEY (from_user_id) REFERENCES users (id);


/**************************************************НАПОЛНЕНИЕ ТАБЛИЦ ДАННЫМИ******************************************************** */

INSERT INTO target_types (id, table_name) VALUES (1, 'users');
INSERT INTO target_types (id, table_name) VALUES (2, 'media');
INSERT INTO target_types (id, table_name) VALUES (3, 'posts');
COMMIT;
...
INSERT INTO likes (target_id,target_type_id,from_user_id,like_status) VALUES
('10','2','55','1'),
('1','1','44','1'),
('5','3','33','0'),
('1','1','36','1'),
...
CREATE INDEX profiles_birth_idx ON profiles(birthday); -- индекс по дате рождения
CREATE INDEX profiles_city_idx ON profiles(city); -- индекс по городу проживания пользователя 
CREATE INDEX profiles_country_idx ON profiles(country); -- индекс по стране проживания пользователя
CREATE INDEX profiles_city_country_idx ON profiles(city, country); -- составной индекс (если вдруг потребуется разбивка данных о профайлах сразу и по городам, и по странам

-- Таблица media
/*Могут быть запросы про объемы файлов в разрезе типов файлов, а также в разрезе конкретных пользователей */

CREATE INDEX media_user_file_size_idx ON media(user_id, size);
CREATE INDEX media_media_type_file_size_idx ON media(media_type_id, size);

-- Таблица messages
/*Наиболее часто запрашиваемыми данными могут быть сообщения ОТ КОГО КОМУ отправлено сообщение */

CREATE INDEX messages_from_user_id_to_user_id_idx ON messages (from_user_id, to_user_id);

-- Таблица likes
/*Наиболее часто запращиваемыми данными могут быть: что поставил пользователь (лайк/дизлайк) для определенного контента определенного типа */

CREATE INDEX likes_from_user_id_target_targettype_id_idx ON likes (from_user_id, target_id, target_type_id);
...
/*РЕШЕНИЕ  */
/*Для рещения задачи решила создать несколько групп (по городу проживания/по стране проживания)*/

-----!!!!Самый молодой/самый "старый" пользователь - определяю по полю [birthday]!!!!!

-- 1. Группировка по городу проживания пользователей

   -- для начала обновим данные в поле [city]
   UPDATE profiles SET city='Moscow' where user_id>0 and user_id<20; 
   UPDATE profiles SET city='Minsk' where user_id>=20 and user_id<50;  
   UPDATE profiles SET city='St.Petersburg' where user_id>=50 and user_id<56; 
   UPDATE profiles SET city='Sochi' where user_id>=56 and user_id<70; 
   UPDATE profiles SET city='Krasnodar' where user_id>=70 and user_id<90; 
   UPDATE profiles SET city='Kursk' where user_id>=90;
   COMMIT;
   
  SELECT DISTINCT city AS 'city_name',
  FIRST_VALUE(user_id) OVER(PARTITION BY city ORDER BY birthday desc) AS 'user_id_youngest', -- user_id самого молодого пользователя в группе
  FIRST_VALUE(birthday) OVER(PARTITION BY city ORDER BY birthday desc) AS 'birthday_of_user_id_youngest', -- ДР самого молодого пользователя в группе
  LAST_VALUE(user_id) OVER(PARTITION BY city
  RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS 'user_id_oldest', -- user_id самого старого пользователя в группе
  LAST_VALUE(birthday) OVER(PARTITION BY city
  RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS 'birthday_of_user_id_oldest', -- ДР самого старого пользователя в группе
  COUNT(*) OVER(PARTITION BY city) AS 'city_count',
  COUNT(*) OVER() AS 'total',
  ROUND((COUNT(*) OVER(PARTITION BY city) / COUNT(*) OVER() * 100),0) AS '%'
  FROM profiles;
  
-- 2. Группировка по стране проживания пользователей

  -- для начала обновим данные в поле [country]. 
  
  UPDATE profiles SET country='Russia' where city != 'Minsk'; 
  UPDATE profiles SET country='Belarus' where city = 'Minsk'; 
  COMMIT;
  
  
  SELECT DISTINCT country AS 'country_name',
  FIRST_VALUE(user_id) OVER(PARTITION BY country ORDER BY birthday desc) AS 'user_id_youngest', -- user_id самого молодого пользователя в группе
  FIRST_VALUE(birthday) OVER(PARTITION BY country ORDER BY birthday desc) AS 'birthday_of_user_id_youngest', -- ДР самого молодого пользователя в группе
  LAST_VALUE(user_id) OVER(PARTITION BY country
  RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS 'user_id_oldest', -- user_id самого "старого" пользователя в группе
  LAST_VALUE(birthday) OVER(PARTITION BY country
  RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS 'birthday_of_user_id_oldest', -- ДР самого "старого" пользователя в группе
  COUNT(*) OVER(PARTITION BY country) AS 'country_count',
  COUNT(*) OVER() AS 'total',
  ROUND((COUNT(*) OVER(PARTITION BY country) / COUNT(*) OVER() * 100),0) AS '%'
  FROM profiles;
